...
export function calculateOxygenGeneratorRating(bits: number[][]) {
  ...
  while (pointer < gammaArray.length) {
    reducedArray = reducedArray.filter(x => x[pointer] === gammaArray[pointer]);
...
export function calculateCO2ScrubberRating(bits: number[][]) {
  ...
  while (pointer < gammaArray.length) {
    reducedArray = reducedArray.filter(x => x[pointer] === epsilonArray[pointer]);
    gammaArray = calculateGammaFromBits(reducedArray);
...
export function altPart2(input: string) {
  ...
  return parseDecimal(oxygenGeneratorRating) * parseDecimal(CO2ScrubberRating);
}

if (require.main === module) {
  const input = fs.readFileSync("./day3.txt", "utf-8");
  console.log(`Day 3, Part 1: ${altPart1(input)}`)
  console.log(`Day 3, Part 2: ${altPart2(input)}`)
}